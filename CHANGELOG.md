...
- [Feature] Full "Up Next" queue view — tapping the "Up next" line in the podcast player now opens the whole queue instead of just naming one episode. From there you can jump straight to any episode, reorder the queue with up/down controls, or remove an episode you don't want to hear, on both mobile and desktop.
...
- [Perf] Inbox search no longer re-runs the full filter/sort pipeline on every keystroke — the search term is now debounced (150ms) before it feeds the item list, so typing stays smooth on larger inboxes. Also split the item-filtering memo so saving/unsaving an article no longer forces an unrelated full recompute of the main feed view (it was only needed for the "Saved" filter, but the dependency was shared across both).
- [Feature] Bulk operations on Cards — a new "Select" mode in the theme-detail view lets you multi-select highlight-cards and bulk-delete or bulk-add a tag across all of them at once (mirrors the same multi-select bar pattern already used in the Inbox), closing one of the biggest gaps vs. Readwise's library management as your card count grows.
...
- [UX] Reeder-style refresh, round 2 — the chrome layer. All modals (Add feed, Smart feed, Folder), mobile sheets (feed drawer, search overlay), the podcast mini-players, reader panels (selection toolbar, highlights drawer, note panel), and shared primitives (Card/Button/Input, search dropdown) now sit on the shared `SHAPE` scale: `radiusCard` surfaces, `radiusSm` controls, unified float shadows, and `accentSurface` active/touch states. Podcast mini-players match the floating nav pill (translucent card + blur) with zero playback-logic lines touched. New directional shadow tokens (`shadowFloatUp`/`shadowFloatLeft`) so bottom sheets and side drawers cast shadows the physically correct way. Design language only — no Reeder assets, icons, or trademarks copied; all colors remain our own theme tokens.

- [UX] Reeder-style UI refresh across the core screens. New shared `SHAPE` scale in `tokens.js` (radii, row spacing, floating-chrome blur/shadow) so all components round and space consistently. Feed list rows get roomier padding, `radiusMd` thumbnails, a uniform 8px accent unread dot, and quieter snippet text. The mobile bottom nav is now a detached floating pill with translucent blurred background (deliberately reinstating the blur removed in 1.46.482 — it is the signature of the Reeder look this refresh targets). Sidebar active states unify on a rounded `accentSurface` pill with calmer label colors and larger section headers. Reader body line-height tightens to 1.68 with all toolbar buttons, popovers, and article images aligned to the shared radius scale. Also fixes a latent bug where the bottom-nav border never rendered (`hexToRgba` was fed an `rgba()` string, producing an invalid color).
...
- [UX] SearchBar: add aria-label, aria-haspopup, aria-expanded, aria-controls linking input to results listbox — screen readers now announce when search results appear

## [1.46.397] — 2026-06-10

- [UX] TagsInput: add aria-label, aria-autocomplete, aria-expanded — screen reader users can navigate tag suggestions
...
- [Perf] useBreakpoint: track breakpoint category string instead of raw pixel width — eliminates unnecessary re-renders in 10+ components on every resize event
...
- [Perf] Add willChange: transform to BottomNav pill — promotes the element to a GPU layer so scroll-direction hide/show animations are composited instead of repainted
...
- v1.46.139
- v1.46.139
- v1.46.138 — Feature: AI morning brief on Today page
- v1.46.136
- v1.46.136
