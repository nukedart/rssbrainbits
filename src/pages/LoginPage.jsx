...
import FeedboxMark from "../components/FeedboxMark";
...
function MagicLinkForm({ onSend, T }) {
  ...
  if (sent) {
    return (
      <div style={{ fontSize:13, color:T.textSecondary, textAlign:"center", padding:"8px 0", lineHeight:1.6 }}>
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:8 }}>
      ...
      <button type="submit" disabled={loading} style={{
        ...
        cursor:loading?"wait":"pointer", fontFamily:"inherit", opacity:loading?0.6:1,
      }}>
        {loading ? "Sending…" : "Send magic link"}
      </button>
      {error && (
        <div style={{ fontSize:12, color:"#e5484d", textAlign:"center" }}>{error}</div>
      )}
    </form>
  );
}
      ...
      <div style={{ maxWidth:400, width:"100%" }}>
        ...
        <div style={{ textAlign:"center", marginBottom:32 }}>
          <div style={{
            ...
            <FeedboxMark size={26} fill={BRAND.mint} />
          </div>
        ...
        <div style={{
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10, margin:"18px 0" }}>
            ...
            <span style={{ fontSize:11, color:T.textTertiary, textTransform:"uppercase", letterSpacing:".05em" }}>or</span>
            <div style={{ flex:1, height:1, background:T.border }} />
          </div>

          <MagicLinkForm onSend={signInWithMagicLink} T={T} />
        </div>
